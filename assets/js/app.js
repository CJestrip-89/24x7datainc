...
(function ($) {
  window.WPHJS = {
    ...
    getContainerInnerWidth: function () {
      var $container = $('<div class="container"><div></div></div>').appendTo(
        $("body")
      );
...
(function ($) {
  ...
  $.fn.calendar = function (options) {
    ...
    var $monthSelector = $('<div class="calendar-month-selector"/>').appendTo(
      $container
    );
    var $weekdaysContainer = $(
      '<div class="calendar-weekdays-container"/>'
    ).appendTo($container);
    var $datesContainer = $('<div class="calendar-dates-container"/>').appendTo(
      $container
    );
    var $nativeInput = $(
    ...
    var drawMonthSelector = function (date) {
      $monthSelector
        ...
        .append(
          ...
          $('<div class="current-month"/>').text(
            options.months[date.getMonth()].full + " " + date.getFullYear()
          ),
          $(
      ...
      if (
        options.minDate &&
        compareDates(options.minDate, lastDayOfMonth(prevMonth)) === 1
      ) {
      ...
      if (
        options.maxDate &&
        compareDates(options.maxDate, firstDayOfMonth(nextMonth)) === -1
      ) {
    ...
    var drawWeekdays = function () {
      ...
      for (var i = 0; i < options.weekdays.length; i++) {
        ...
        $weekdaysContainer.append(
          $('<div class="weekday"/>').text(options.weekdays[index].short)
        );
      }
    };
      ...
      for (var i = 0; i < weeks.length; i++) {
        ...
        for (var j = 0; j < currentWeek.length; j++) {
          ...
          if (compareDates(currentDate, lastDayOfMonth(currentDate)) === 0) {
            ...
            compareDates(currentDate, firstDayOfMonth(currentDate)) === 0
          ) {
          ...
          if (
            options.selectedDay &&
            compareDates(currentDate, options.selectedDay) === 0
          ) {
          ...
          if (
            (options.maxDate &&
              compareDates(currentDate, options.maxDate) === 1) ||
            (options.minDate &&
              compareDates(currentDate, options.minDate) === -1)
          ) {
            classNames.push("disabled");
          }

          var $dayItem = $("<div/>", { class: classNames.join(" ") })
            .data("date", currentDate)
            .appendTo($datesContainer);
          var $dayItemContent = $("<div/>", {
          ...
          $dayItemContent.append(
            $.map(options.dayItemContents, function (val) {
              if (val === "month-full") {
                ...
                return $('<div class="day"/>').text(currentDate.getDate());
    ...
    $nativeInput.on("change", function () {
      ...
      displayDate = new Date(
        options.selectedDay.getFullYear(),
        options.selectedDay.getMonth()
      );
      drawMonthSelector(displayDate);
      drawDates(displayDate);
...
(function ($) {
  ...
  var processSliderViewportMode = function (e) {
    var containerInnerWidth = WPHJS.getContainerInnerWidth(),
      ...
      gapValue =
        windowWidth < 576 ? 0 : (windowWidth - containerInnerWidth) / 2;
  ...
  $sliders
    .filter(".card-slider-viewport")
    .on("ready.flickity", processSliderViewportMode);
  ...
  $('[data-target][data-action][data-toggle="card-slider"]').each(function () {
    ...
    $slider.on("ready.flickity change.flickity", function () {
      var that = this;
      setTimeout(function () {
        // schedule code for next tick when `data('flickity')` will be available
        ...
        $control.toggleClass(
          "disabled",
          !flky.options.wrapAround && flky.selectedIndex === boundIndex
        );
        $control.prop(
          "disabled",
          !flky.options.wrapAround && flky.selectedIndex === boundIndex
...
(function ($) {
  ...
  if (!$spacingHelper.length) {
    $spacingHelper = $('<div class="header-spacing-helper"/>').insertAfter(
      $header
    );
  }
...
(function ($) {
  ...
  $sliders
    .find(".image-slider-prev, .image-slider-next")
    .on("click", function (e) {
      var $control = $(this),
        $slider = $control
  ...
  $sliders.find(".image-slider-contents").flickity({
    ...
    prevNextButtons: false,
    pageDots: false,
    autoPlay: 4000, // set autoPlay to 4 seconds (4000 milliseconds)
    pauseAutoPlayOnHover: false, // optional: pause on hover
  });
})(jQuery);
...
jQuery(window).one("wph.google_maps_loaded", function () {
  ...
  $mapBlocks.each(function () {
    var $map = $(this).find(".gmap-embed-inner"),
      ...
      parallax_speed = parseInt(
        $(this).find(".gmap-embed").data("parallax-speed")
      );
    ...
    $($infoboxes.get().reverse()).each(function () {
      ...
      var infobox = new GoogleMaps_InfoBox_Factory({
        ...
        pixelOffset: new google.maps.Size(
          57,
          ($infobox.find(".gmap-infobox-header").outerHeight() / 2) * -1
        ),
        closeBoxURL: "",
        zIndex: 1,
...
(function ($) {
  ...
  if (typeof google === "undefined" || typeof google.maps === "undefined") {
    var g_api_url =
      "https://maps.google.com/maps/api/js?key=" + WPHJS.GOOGLE_MAPS_API_KEY;
...
(function ($) {
  ...
  $(".radio-buttons-group:not(.radio-buttons-group-disabled)").each(
    function () {
...
(function ($) {
  ...
  $(".shuffle-grid")
    ...
    .on("set-filter.shufflejs", function (e, filterValue) {
      var shuffle = $(this).closest(".shuffle-grid").data("shufflejs-instance");

      shuffle.filter(
        ["all", "*"].indexOf(filterValue) !== -1
          ? Shuffle.ALL_ITEMS
          : filterValue
      );
    });
  ...
  $('.radio-buttons-group[data-toggle="shuffle-grid"][data-target]').on(
    "change",
    function (e, filterValue) {
      var $control = $(this),
